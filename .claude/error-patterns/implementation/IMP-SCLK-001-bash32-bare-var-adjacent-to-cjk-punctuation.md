...
以上為可複現的外部行為。至於「為何 UTF-8 locale 才發生」（bash 3.2 不完整的多位元組支援，或 locale 的字元分類差異），未讀 bash 原始碼前不宜斷言，此處只記錄觀測規則。
...
| 軸 | 不觸發的一側 |
|----|------------|
| bash 版本 | 開發者本機可能裝了 homebrew bash 4/5 |
| locale | CI 環境常預設 `C` / `POSIX` locale |

複現失敗時第一個該查的是 locale，其次才是 bash 版本。
