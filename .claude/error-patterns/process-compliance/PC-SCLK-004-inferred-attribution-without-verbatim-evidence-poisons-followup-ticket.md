...
用一個對照更精確：**若丟一枚硬幣猜正反，猜中不代表這是一次有依據的判斷**。W1-015 的診斷之所以在方法論上不合格，不是因為它錯了，而是因為它在沒有逐字證據的情況下就被寫成確定語氣的結論，並被後續兩次回報（W1-001.4、W1-001.5）直接引用——W1-001.5 甚至生成了另一個互斥版本（「跨 worktree git 操作」），**兩個版本不可能同時為真，但兩者在各自報告當下都被寫成同樣篤定的語氣**。這正是本模式要防的：篤定語氣與證據強度脫鉤，讀者無法從文字本身分辨這是「已驗證」還是「合理推測」。
