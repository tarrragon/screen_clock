...
> **核心摘要**：Ticket 是「最小可交付單元」，與工作日誌互補（Ticket 管執行層面、worklog 管記錄層面）。設計遵循三大目標——可追溯性、可驗收性、可協作性。拆分標準是 Atomic Ticket 的單一職責四大檢查（禁用量化指標）。執行中積極派發子 Ticket 建立任務鏈，不追求單一 Ticket 完成所有任務。
