...
分析導向的教學模組（判準密集、讀者要帶走判斷力的內容），模組級結構要是**推導體系**、不是主題集合：一個源頭機制（一個成本結構、一個約束、一個生命週期——各篇判準能折算回去的基準），每篇承擔一條從源頭展開的推導線，模組入口能一句話說出「推導的起點是什麼」。

源頭機制買到四件事：各篇判準折算到同一把尺（讀者學一把尺加 N 個用法、不是 N 套規則）、跨篇判準矛盾才會現形（兩篇判準互斥要有共同基準才「是矛盾」）、新篇有明確的掛載點（擴篇時先問「新篇折回源頭的路徑」）、推導式的閱讀路線成立。主題集合式的模組被批評「淺」時無從補起——沒有源頭就沒有「往哪裡深」的方向。

邊界：目錄型模組（工具清單、vendor overview）與異質 case 記錄不適用，硬找源頭是強加敘事；源頭是判準的折算基準、不是每篇的開場模板——寫成固定開場會複製 cadence 同質化。詳見 [teaching-module-needs-derivation-anchor](principles/teaching-module-needs-derivation-anchor.md)。
