...
> **角色**：本卡是 `compositional-writing` 的支撐型原則（principle）、被 SKILL.md 第 6 原則「Multi-pass Review」明示引用、被 `references/writing-articles.md` / `writing-code-comments.md` / `writing-prompts.md` / `writing-documents.md` 在「自檢不是 multi-pass」段引用、是「為什麼寫作品質要靠 multi-pass review、不靠單一 lint / hook 攔截」的根據。
>
...
| 第 2 輪：自查 / 對比需求      | 邏輯偏差、漏 case    | 對比原始需求、列 checkpoint                 |
...
| 原則                                                                            | 關係                                                                                               |
| ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| [methodology-multi-pass-embedding](./methodology-multi-pass-embedding.md)       | 本卡是「方法論設計本身」這一層的展現 — multi-pass 升 pillar 才結構性執行                           |
| [writing-multi-pass-review](./writing-multi-pass-review.md)                     | 本卡在「寫」這個動作上的具體實例 — review 是 multi-pass、不是 hook                                 |
| [naming-as-iterated-artifact](./naming-as-iterated-artifact.md)                 | 命名 lint（max length、case style）只擋字面、grep-ability / 一致性 / impl 洩漏靠 multi-pass review |
| [ease-of-writing-vs-intent-alignment](./ease-of-writing-vs-intent-alignment.md) | 行為錯誤多半源自「便利驅動」的偏移、單次寫不出對齊                                                 |

本卡跟「外部觸發 / 結構性對策」原則互補：字面用 L5 hook、行為用 L4 pair + multi-pass。
