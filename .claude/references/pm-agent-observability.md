...
| **SendMessage shutdown_request** | 我如何主動終止一個 idle named agent？ | 主動請求（tool call） | PM 判斷 idle agent 應放生時 | PM 呼叫，CC runtime 執行 |

**設計原則**：六工具**互補非替代**。PM 應在不同場景選擇對應工具，禁止以單一工具推論全部資訊。
...
### 5. SendMessage shutdown_request（idle agent 放生，W1-008 ANA 落地）
...
SendMessage(
  to: "<agent-name>",
  message: {"type": "shutdown_request", "reason": "<放生理由，如 Wave 1 同類型 ticket 已全數完成>"}
)
```

**返回值**：`SendMessage` 呼叫本身回傳 `success` 與 `request_id`（格式 `shutdown-<timestamp>@<agent-name>`），代表 PM 端請求已送達 CC runtime。

**驗證狀態**：已驗證（2026-07-08 實機 8/8：PM 端發送成功 + agent 端全數 approve 終止）。Wave 1 收尾對 8 個 idle agent（thyme-w1-001/002/005/006/007/009、basil-w1-004、saffron-w1-008）批次發送 `shutdown_request`，全數收到 `shutdown_approved` 回應並觀察到 `teammate_terminated` 事件，證實協議在 CC runtime 端到端可用（PM 端請求送達 + agent 端終止執行皆有直接觀測證據，非僅 schema 文件存在或間接推論）。

**限制**：
- 續用（新任務派發）與放生（shutdown_request）為互斥動作，PM 需先完成二分判斷（見 `.claude/pm-rules/parallel-dispatch.md`「idle agent 回收 SOP」）再呼叫
- 8/8 為單一 Wave 收尾場景的實機驗證，尚未涵蓋跨 session / 跨版本長時間累積場景下的行為
...
**Version**: 1.5.0 - 五工具擴為六工具：新增「SendMessage shutdown_request」章節（idle agent 放生協議，schema/範本/驗證層級標注/限制），決策樹新增 idle 續用/放生分支（W1-008 ANA 落地，W1-010）
