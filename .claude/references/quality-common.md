...
### 1.8 依賴引入紀律（新增執行期依賴時強制）

> **觸發時機**：準備在專案加入新的執行期第三方套件（npm / pub / pip / go get 等）時。**邊界**：本節管「執行期依賴」（會成為產物一部分、需長期維護的套件）；開發期工具（linter、測試框架）與 Claude Code plugin 的引入評估另見 `.claude/references/plugin-management.md`。

**核心原則**：每個依賴都是你不掌控、卻永久成為專案一部分的碼——需維護、更新、資安稽核、被團隊每個人理解。成本幾乎總是高於表面。加之前先窮盡既有選項。

**加套件前四問**（任一為「是」即應避免新增）：

| 問題 | 生態對照範例 | 為「是」時的行動 |
|------|-------------|----------------|
| 1. 專案既有依賴能做到嗎？ | 已有 axios 就別加 node-fetch；已有 date-fns 就別加 moment | 用既有依賴 |
| 2. 標準庫能做到嗎？ | `Array.prototype.map` 不需 lodash；`crypto.randomUUID()` 不需 uuid | 用標準庫 |
| 3. 這套件仍在維護嗎？ | 查最後 commit 日期、issue 數、維護者是否回應 | 未維護則另尋或自寫 |
| 4. 體積是否與收益相稱？ | 為格式化日期加 500KB 套件不相稱 | 不相稱則自寫最小實作 |

**必附理由**：確實需要新增時，在 Ticket / commit 說明「為何非加不可」——例：「加 zod 因需 runtime schema 驗證，既有依賴無此能力」。**禁止**靜默把套件塞進 manifest（package.json / pubspec.yaml / requirements.txt）而不交代理由。

**Why/Consequence**：靜默新增依賴讓 review 者無從評估必要性，供應鏈風險（未維護套件、typosquatting）在無人把關下進入專案。**Action**：新增執行期依賴必經四問 + 附理由；安裝指令須在 fresh shell 驗證且用完整套件名（避免短名被 squat），詳見 `.claude/error-patterns/process-compliance/PC-159-*`（fresh shell 驗證 / @scope 全名）。
...
- [ ] 配置與程式碼分離
- [ ] 無重複程式碼（DRY，重複率 < 10%）
- [ ] 新增執行期依賴已過四問（既有 / 標準庫 / 維護狀態 / 體積）並附理由（1.8）
...
**Version**: 1.8.0 — 新增 §1.8「依賴引入紀律」（1.5.0-W5-009.2 落地，源自外部 CLAUDE.md 範例第 8 章）：加套件前四問（既有 / 標準庫 / 維護狀態 / 體積）+ 必附理由 + 三明示 + 交叉引用 PC-159（fresh shell 驗證）與 plugin-management（邊界）；§2 常數管理檢查清單同步補項
