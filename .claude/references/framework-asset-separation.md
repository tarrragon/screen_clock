...
| **Skill 私有 Hook** | `.claude/skills/<skill>/hooks/<name>.py` | 僅該 Skill 觸發場景使用 | `.claude/skills/dart-test-async-guardian/hooks/pre-test-scan.py` |
...
- Skill 私有 Hook 路徑已含 Skill 名稱（如 `dart-test-async-guardian/hooks/`），檔名再加 Skill 前綴造成冗餘；簡化為 `<action>-<scope>.py` 即可，閱讀時自然從路徑得知歸屬。
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.claude/skills/dart-test-async-guardian/hooks/pre-test-scan.py"
          }
        ]
      }
    ]
