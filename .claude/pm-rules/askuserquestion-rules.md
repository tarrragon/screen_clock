...
**呈現處置取捨類選項前，選項清單必須取自 WRAP W 階段產出，不得直接使用直覺清單。**

處置取捨的識別特徵：選項在回答「這個東西該留還是該拿掉」，典型形態為保留 / 回退 / 停用 / 刪除的組合，或「維持現狀 vs 改動程度」的程度軸。
...
**Why**：處置類問題天然以對立軸呈現，直覺產生的清單會被「該多硬 / 該留多少」這種程度軸限定，而真正的解常在程度軸之外——移位到別的執行點、換掉判準來源、重新定義防護對象。選項一旦列出即封閉解空間，且該封閉對列選項者不可見：PM 只會看到自己列了三個選項，看不到沒被列出的第四個。

**Consequence**：使用者只能在被限定的清單內選擇，或必須自己看穿框架才能提出第四個選項。決策品質上限由列選項者的框架決定，而非由問題本身決定——這與規則 5.6 的 illusion of control 同族，差別在假選項是「列了不該列的」，選項空間過窄是「沒列該列的」，後者更難察覺因為沒有矛盾訊號可循。

**Action**：識別到正要列處置類選項時停手，執行 `/wrap-decision` 的 W 階段，清單取自其產出。W 階段產出與直覺清單一致時照列即可——最低成本是一次檢查，不是一次重寫。處置涉及防護能力增減（移除守衛、降級閘門、停用檢查）或不可逆變更時，W 階段不足，須跑完整 WRAP（升級條件與理由見 `wrap-decision` SKILL「二元處置取捨」節）。

**與選項前提檢查的分工**：前提檢查是減法（刪除前提不成立的選項），本檢查是加法（補上框架外的選項）。兩者都執行才構成完整的選項品質把關。

> **實證**：某次面對「防護 hook 的出口被降級後該保留還是回退」的處置決策，直覺清單為三項（保留 / 回退 / 先複核）。跑 WRAP 後，W 階段反向框架把問題從「出口該多硬」重寫為「這個防護該不該存在於此執行點」，方案擴增至五項，使用者最終採用的方案不在直覺清單內。
...
| 處置取捨類選項未經 WRAP W 階段產出 | 停止，執行 `/wrap-decision` W 階段後重新產出 AUQ |
