...
### 寫入品質：三類標註（PC-BAL-004）

Context Bundle 的每項陳述須可歸入下列之一，無法歸類者不寫入：
...
**Why**：Context Bundle 位於 Problem Analysis 這種語意上代表「已分析」的章節，且通常附行號、指令、數字，具權威外觀。執行者的預設是採信而非複驗，故未查證的陳述會直接成為實作前提。

**Consequence**：前提錯誤時，執行者花回合修正而非產出；若未察覺而照單全收，錯誤前提固化進實作與後續文件。

**Action**：以 `grep` / `find` / `ls` 的輸出作為事實前，先問「這個 pattern 會不會命中我不想要的東西」。

| 風險 | 檢查 |
|------|------|
| 未錨定子字串比對 | 命中註解、字串常數、文件引用？加 `^` 錨定或改結構化解析 |
| 斷言「不存在」 | 路徑是否打錯？以兩種方式交叉確認（`ls` / `find` / `git log --` 併用） |
| 計數 | 抽樣檢視命中內容，勿只看 `wc -l` |

**否定命題加倍查證**：「X 不存在」「Y 未宣告」的錯誤成本高於肯定命題——它導向「建立 X」的工作方向，而正確方向可能是「更新既有 X」。

**與 PC-162 的分工**：ticket 模板既有的「接手者應獨立重新驗證數量／範圍」是安全網，本節是主責。不得以安全網存在為由降低寫入時的查證強度。
...
**交叉引用**：
- `.claude/references/agent-definition-standard-details.md`「執行責任：Ticket body 填寫」— agent 填 Solution 的主責定義
