---
id: 1.0.0-W3-001
title: macOS LaunchAtLogin 方案評估（ANA）
type: IMP
status: completed
version: 1.0.0
wave: 3
...
who:
  current: pending
  history: {}
what: 說明：比較 (a) `launch_at_startup` 套件、(b) 手寫 launchd plist、(c) macOS 13+ SMAppService。考量未來
  notarization 流程相容性、是否需要使用者授權。產出 SPEC-006 開機啟動規格。
when: 待定義
where:
  layer: 待定義
  files:
  - docs/spec/platform/SPEC-006-macos-auto-launch.md
why: 實作 macOS 開機自動啟動功能。設定面板提供開關，使用者可隨時開啟 / 關閉。
how:
  task_type: macOS
  strategy: 待定義
acceptance:
