...
who:
  current: pending
  history: {}
what: 說明：在乾淨 macOS 環境安裝 .dmg；依 UC-01/02/03 全部驗收條件逐項驗證；確認 code signing 通過 Gatekeeper；CPU
  量測 5 分鐘閒置；產出 v1.0.0 發布記錄。
when: 待定義
where:
  layer: 待定義
  files:
  - docs/work-logs/v1/v1.0/v1.0.0/v1.0.0-main.md
why: v1.0.0 的最後 wave：app icon、README、code signing、notarization、打包 .dmg、最終驗收。
how:
  task_type: v1.0.0
  strategy: 待定義
acceptance:
