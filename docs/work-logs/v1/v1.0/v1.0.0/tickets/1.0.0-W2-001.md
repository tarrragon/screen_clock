...
who:
  current: pending
  history: {}
what: 說明：比較 (a) 右鍵 menu、(b) 鍵盤快捷鍵（Cmd+Opt+,）、(c) macOS menu bar status icon 三種呼出方式。考量遮罩
  click-through 下右鍵點不到，主要候選為 (b) + (c)。產出 SPEC-005 設定面板規格。
when: 待定義
where:
  layer: 待定義
  files:
  - docs/spec/user-experience/SPEC-005-settings-panel-entry.md
why: 提供使用者設定 UI 並處理「全穿透」與「設定面板需互動」的衝突——動態切換 click-through。設定變更即時預覽。
how:
