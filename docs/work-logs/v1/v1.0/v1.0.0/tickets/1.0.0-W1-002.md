...
who:
  current: pending
  history: {}
what: 說明：定義設定欄位（fontSize、fontColor、strokeColor、timeFormat、targetScreenIndex、autoLaunch、position）；提供
  fromJson / toJson；含 default 工廠方法重現 MVP 預設值。
when: 待定義
where:
  layer: 待定義
  files:
  - lib/models/settings_model.dart
why: 把寫死的預設值（樣式、位置、目標螢幕、開機啟動）改為可儲存的設定。本 wave 只做資料層與啟動時套用，UI 在 W2 處理。
how:
  task_type: SettingsModel
  strategy: 待定義
acceptance:
