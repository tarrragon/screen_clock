...
who:
  current: pending
  history: {}
what: 說明：在 `awakeFromNib` 中加入 `self.isOpaque = false; self.backgroundColor = .clear`，這是
  Flutter 端 transparent 真實生效的前提（SPEC-001 FR-02 設計約束）。
when: 待定義
where:
  layer: 待定義
  files: []
